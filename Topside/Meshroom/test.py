...
def camera_worker(camera_id, url, shm_name, sync_dict, interrupt_event):
    ...
    # Attach to pre-allocated shared memory
    existing_shm = shared_memory.SharedMemory(name=shm_name)
    # View memory space directly as a NumPy array (Zero-Copy)
    ...
    try:
        while not interrupt_event.is_set():
            ...
            if r and f is not None:
                ...
                # Fast, in-place memory copy inside the shared memory segment
                np.copyto(shared_array, f)
                
                # Atomically update metrics without passing whole matrices
            ...
            if (time.time() - last_heartbeat) > timeout_threshold:
                ...
                time.sleep(1.0)
                last_heartbeat = time.time() # Prevent spin log flooding
...
def inference_worker(raw_shm_name, out_shm_name, sync_dict, interrupt_event):
    ...
    """
    # Load model context cleanly inside child memory space
    model = YOLO("best.pt")
    ...
    try:
        while not interrupt_event.is_set():
            ...
            # Process only if a brand new camera frame arrived
            if current_tick != last_tick:
                last_tick = current_tick
                
                inf_start = time.time()
                # Run prediction directly from the shared memory reference
                ...
                # Push back annotated image array to output shared segment
                np.copyto(out_array, annotated)
                sync_dict["inference_time"] = inf_duration
            else:
                time.sleep(0.002) # Don't melt CPU core waiting for next frame
...
def telemetry_logger(sync_dict, interrupt_event, filename="vision_performance.csv"):
    ...
    while not interrupt_event.is_set():
        ...
        # Calculate latencies using shared timestamps
...
# -----------------------------------------------------------------------------
# MAIN APPLICATION EXECUTIVE EXECUTION
# -----------------------------------------------------------------------------
if __name__ == '__main__':
    # Ensure spawn context for safe multiprocessing across platforms containing deep learning links
    ...
    # Initialize basic latency tracker metrics 
    for i in range(4):
        sync_dict[f"lat_{i}"] = time.time()
        sync_dict[f"frame_tick_{i}"] = 0
    sync_dict["inference_time"] = 0.0

    # Allocate physical shared buffers inside RAM
    shm_buffers = []
    cam_shm_names = []
    for i in range(4):
        ...
        # Clear/Zero initialize buffer array 
        arr = np.ndarray(FRAME_SHAPE, dtype=np.uint8, buffer=shm.buf)
        arr[:] = 0

    # Setup specific isolated output matrix storage for the AI channel
    ...
    # Local pointers inside main context for displaying UI
    ...
    # Launch Video Capture Workers
    urls = [0, 0, 0, 0]  # Change indices or strings depending on camera bindings
    processes = []
    ...
    # Launch Inference Worker (Tracks Camera 0 Shared Segment Name)
    ...
    try:
        while not interrupt_event.is_set():
            # Zero-Allocation: Read frames directly out of local memory window views
            # Camera 0 draws out of the specialized AI processing buffer pipeline view
            ...
            # Rate-limiting the UI frame loop to 60Hz preserves massive CPU availability
            ...
            if key == ord('q'):
                ...
                # Use standard direct view rather than heavy multi-lock frame extraction copy commands
        ...
        # Explicit clean termination of background workers
        for p in processes:
            if p.is_alive():
                p.terminate()
                p.join(timeout=1.0)

        # Force kill Photogrammetry if active
        ...
        # Free Shared Memory slots back to operating system allocations
